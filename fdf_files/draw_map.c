...
/*   Updated: 2022/11/21 19:39:32 by mmensing         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
double	get_point(double prev_x, double prev_y, int32_t prev_z, char *x_or_y)
{
	if (ft_strncmp(x_or_y, "x", 1) == 0)
		return (START_POINT_X + prev_x * (TILE_WIDHT / LINE_LEN) - prev_y * \
				(TILE_WIDHT / LINE_LEN));
	else
		return ((START_POINT_Y + prev_x * (TILE_HIGHT / LINE_LEN) + prev_y * \
				(TILE_HIGHT / LINE_LEN)) - prev_z);
}
